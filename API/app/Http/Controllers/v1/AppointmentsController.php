...
class AppointmentsController extends Controller
{
    public function save(Request $request){
        ...
        $data = EventContract::create($request->all());
        if (is_null($data)) {
            $response = [
            'data'=>$data,
            'message' => 'error',
    ...
    public function getAllSalonAppointments(Request $request){
        $data = EventContract::where('salon_id','!=',0)->orderBy('id','desc')->get();
    ...
    public function getAllFreelancerAppointments(Request $request){
        $freelancerData = EventContract::where('id','!=',0)->orderBy('id','desc')->get();
        foreach($freelancerData as $loop){
    ...
    public function getMyList(Request $request){
        ...
        $data = EventContract::where('uid',$request->id)->orderBy('id','desc')->get();
        foreach($data as $loop){
    ...
    public function getSalonList(Request $request){
        ...
        $data = EventContract::where('salon_id',$request->id)->orderBy('id','desc')->get();
        foreach($data as $loop){
            $loop->salonInfo = Salon::where('uid',$loop->salon_id)->first();
            $loop->userInfo = User::select('id','first_name','last_name','cover')->where('id',$loop->uid)->first();
        }
    ...
    public function getIndividualList(Request $request){
        ...
        $data = EventContract::where('freelancer_id',$request->id)->orderBy('id','desc')->get();
        foreach($data as $loop){
            $loop->individualInfo = DB::table('individual')
                ->select('individual.*','users.first_name as first_name','users.last_name as last_name')
                ->join('users','individual.uid','users.id')
    ...
    public function getInfoAdmin(Request $request){
        ...
        $data = EventContract::find($request->id);
        $data->userInfo = User::where('id',$data->user_id)->first();
        if($data->individual_uid  == 0){
            $data->salonInfo = Salon::where('uid',$data->salon_id)->first();
            $data->ownerInfo = User::select('first_name','last_name','email','mobile','country_code','fcm_token','cover')->where('id',$data->salon_id)->first();
        }else{
            $data->individualInfo = DB::table('individual')
            ->select('individual.*','users.first_name as first_name','users.last_name as last_name')
            ->join('users','individual.uid','users.id')
            ->where('individual.uid',$data->individual_uid )
            ->first();
            $data->ownerInfo = User::select('first_name','last_name','email','mobile','country_code','fcm_token','cover')->where('id',$data->individual_uid )->first();
        }
        $response = [
            'data'=>$data,
            'success' => true,
    ...
    public function getStats(Request $request){
        ...
        if($request->type == 'individual'){
            $monthData = EventContract::select(\DB::raw("COUNT(*) as count"), \DB::raw("DATE(save_date) as day_name"), \DB::raw("DATE(save_date) as day"),\DB::raw('SUM(total) AS total'))
    ...
    public function getMonthsStats(Request $request){
        ...
        if($request->type == 'individual'){
            $monthData = EventContract::select(\DB::raw("COUNT(*) as count"), \DB::raw("MONTH(save_date) as day_name"), \DB::raw("MONTH(save_date) as day"),\DB::raw('SUM(total) AS total'))
                ->whereYear('save_date', $request->year)
                ->groupBy('day_name','day')
                ->orderBy('day')
                ->where('freelancer_id',$request->id)
    ...
    public function getAllStats(Request $request){
        ...
        if($request->type == 'individual'){
            $monthData = EventContract::select(\DB::raw("COUNT(*) as count"), \DB::raw("YEAR(save_date) as day_name"), \DB::raw("YEAR(save_date) as day"),\DB::raw('SUM(total) AS total'))
                ->groupBy('day_name','day')
                ->orderBy('day')
                ->where('freelancer_id',$request->id)
                ->get();
    ...
    public function getByDate(Request $request){
        ...
        if($request->type == 'individual'){
            $data = EventContract::where('freelancer_id',$request->id)->whereDate('save_date',$request->date)->orderBy('id','desc')->get();
            foreach($data as $loop){
                $loop->individualInfo = DB::table('individual')
                    ->select('individual.*','users.first_name as first_name','users.last_name as last_name')
                    ->join('users','individual.uid','users.id')
        ...
        if($request->type == 'salon'){
            $data = EventContract::where('salon_id',$request->id)->whereDate('save_date',$request->date)->orderBy('id','desc')->get();
            foreach($data as $loop){
                $loop->salonInfo = Salon::where('uid',$loop->salon_id)->first();
                $loop->userInfo = User::select('id','first_name','last_name','cover')->where('id',$loop->uid)->first();
            }
    ...
    public function getAppointmentsSalonStats(Request $request){
        ...
        $to = date($request->to);
        $data = EventContract::whereRaw('FIND_IN_SET("'.$request->id.'",salon_id)')->whereBetween('save_date',[$from, $to])->where('status',4)->orderBy('id','desc')->get();
    ...
    public function getAppointmentsFreelancersStats(Request $request){
        ...
        $from = date($request->from);
        $to = date($request->to);
        $data = EventContract::whereRaw('FIND_IN_SET("'.$request->id.'",freelancer_id)')->whereBetween('save_date',[$from, $to])->where('status',4)->orderBy('id','desc')->get();
        $commission = DB::table('commission')->select('rate')->where('uid',$request->id)->first();
        $response = [
            'data'=>$data,
            'commission'=>$commission,
