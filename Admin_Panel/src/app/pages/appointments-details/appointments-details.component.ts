...
export class AppointmentsDetailsComponent implements OnInit {
  ...
  orderStatusNames = [
    ...
    this.util.translate('pending'),// 1
    this.util.translate('Declined'), // 2
     // 3
  ...
  toggleEditMode() {
    this.editMode = !this.editMode; // Step 3
  }
    ...
    this.api.post_private('v1/appoinments/getInfoAdmin', { id: this.id }).then((data: any) => {
      ...
      if (data && data.status && data.status == 200 && data.data && data.data.id) {
        ...
        if (info.individualInfo) {
          this.freelancerName = info.musician;
          this.freelancerCover = info.ownerInfo.cover;
          this.freelancerEmail = info.ownerInfo.email;
          this.freelancerMobile = info.ownerInfo.mobile;
        ...
        this.notes = info.extra_field;
        this.paid = info.paid;
        this.pay_method = info.payment_method;
        this.save_date = info.date;
        this.serviceTax = info.body;
        this.slot = info.time;
  ...
  saveChanges() {
    ...
    const modifiedData = {
      ...
      time: this.slot,
      fee: this.total,
      wallet_price: this.wallet_price,
      // ... other properties you want to update
  };
  
    // Make an HTTP request to save the modified data
    this.api.post_private('v1/event_contract/update', modifiedData)
    .then((response: any) => {
        // Check the server response
        if (response && response.status === 200) {
            console.log('Data saved successfully:', response);
            // Optionally, you may want to update any local variables or UI elements
            // based on the server response
        } else {
            console.error('Failed to save data. Server response:', response);
        }
    })
    .catch((error: any) => {
  ...
  changeOrderStatus() {
    ...
    // Assuming orderStatus is a variable that stores the current status of the order
    // And changeStatusOrder is the variable bound to the dropdown value
  
    // Check the selected status and call the appropriate refund method
    if (this.changeStatusOrder === 'pending') {
      console.log('refund with API');
      // Call refund method based on the payment method
      this.refundOrder();
    } else if (this.changeStatusOrder === 'Accepted') {
      console.log('Order accepted');
      // Call another method related to accepting orders
      this.refundPayPal();
    } else if (this.changeStatusOrder === 'Declined') {
      console.log('Order declined');
      // Call another method related to declining orders
      this.refundPayTM();
    } else {
