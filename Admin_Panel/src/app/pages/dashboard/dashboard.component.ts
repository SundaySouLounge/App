...
export class DashboardComponent implements OnInit {
  ...
  getHome() {
    ...
    this.api.get_private('v1/freelancer/getAdminHome').then((data: any) => {
      ...
      if (data && data.status && data.status == 200) {
        ...
        this.appointments = data.appointments.sort((a: any, b: any) => {
  ...
  getAllNotifications() {
    console.log('Calling getAllNotifications() method...');
    this.api.get_private('v1/notification/getAllNotifications').then((data: any) => {
        console.log('Data:', data);
        if (Array.isArray(data.notifications)) {
            this.notifications = data.notifications;
        } else {
            console.error('Notifications array not found in data:', data);
        }
    }, error => {
