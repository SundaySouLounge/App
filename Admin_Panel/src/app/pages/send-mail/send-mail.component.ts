...
export class SendMailComponent implements OnInit {
  ...
  save() {
    ...
    if (this.sendTo == 1) {
        ...
        this.sendToStores();
    } else if (this.sendTo == 4) {
        this.sendToDrivers();
    } else if (this.sendTo == 5) { // New condition for sending to individual
        this.sendToIndividual();
    }
}

// Method to send email to an individual
sendToIndividual() {
    if (!this.individualEmail || this.individualEmail.trim() === '') {
        this.util.error(this.util.translate('Please enter an email address'));
        return;
    }
    ...
    const param = {
        ...
        content: this.message,
        email: this.individualEmail.trim()
    };

    this.util.show();
    this.api.post_private('v1/users/sendMailToIndividual', param).then((data: any) => {
        console.log(data);
        this.util.hide();
        if (data) {
            this.util.success(this.util.translate('Email sent'));
